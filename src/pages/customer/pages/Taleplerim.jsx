...
const requestsData = [
  {
    ...
    date: '2026-02-25', // tarih
    time: '09:00 - 17:00', // saat
    location: 'İstanbul, Kadıköy', // konum
    status: 'active', // durum: aktif
    caregiver: 'Fatma Demir', // bakıcı
    notes: 'Annem için düzenli bakım hizmeti.', // notlar
  },
  {
    id: 'TLP-2026-002',
...
const statusConfig = {
  active: { bg: '#ECFDF5', text: '#059669', label: 'Aktif', icon: CheckCircle }, // aktif
  pending: { bg: '#FFFBEB', text: '#D97706', label: 'Beklemede', icon: Clock }, // beklemede
  completed: { bg: '#F0F0FF', text: 'var(--color-primary)', label: 'Tamamlandı', icon: CheckCircle }, // tamamlandı
  cancelled: { bg: '#FEF2F2', text: '#DC2626', label: 'İptal', icon: AlertCircle }, // iptal
};
...
export default function Taleplerim() {
  ...
  const [requests, setRequests] = useState(requestsData); // talepler
  const [search, setSearch] = useState(''); // arama
  const [filter, setFilter] = useState('all'); // filtre

  // Yeni talep ekleme (Add new request)
  ...
  const filtered = requests.filter((r) => {
    const matchesSearch = // arama eşleşmesi
      !search ||
      r.service.toLowerCase().includes(search.toLowerCase()) ||
      r.id.toLowerCase().includes(search.toLowerCase()) ||
      (r.caregiver && r.caregiver.toLowerCase().includes(search.toLowerCase()));
    const matchesFilter = filter === 'all' || r.status === filter; // filtre eşleşmesi
    ...
    <CustomerLayout title="Taleplerim - BakıcıKolay">
      ...
      <div className="bg-white rounded-[var(--radius-md)] border border-[var(--color-border)] mb-6">
        <div className="flex flex-col sm:flex-row sm:items-center gap-3 p-4">
          ...
          {/* Durum filtresi (Status filter tabs) */}
          <div className="flex gap-1 bg-[var(--color-bg)] rounded-[var(--radius-sm)] p-1">
            {[
              { key: 'all', label: 'Tümü' }, // tümü
              { key: 'active', label: 'Aktif' }, // aktif
              { key: 'pending', label: 'Beklemede' }, // beklemede
              { key: 'completed', label: 'Tamamlandı' }, // tamamlandı
              { key: 'cancelled', label: 'İptal' }, // iptal
            ].map((f) => (
              <button
                key={f.key}
